...
export default function EditEmbossingOptionPage() {
  ...
  const [embossingOption, setEmbossingOption] = useState<EmbossingOption | null>(null);
  ...
  useEffect(() => {
    if (id) {
      fetchEmbossingOption();
    }
  }, [id]);

  const fetchEmbossingOption = async () => {
    try {
      setLoading(true);
      const response = await apiRequest('GET', `/api/admin/embossing-options/${id}`);
      const data = await response.json();
      setEmbossingOption(data);

      // Populate form
      setFormData({
        name: data.name || '',
        slug: data.slug || '',
        price: data.price?.toString() || '80',
      ...
      toast({
        title: 'Error',
        description: 'Failed to fetch embossing option',
        variant: 'destructive'
      });
  ...
  const handleInputChange = (field: string, value: any) => {
    if (field === 'name') {
      // Auto-generate slug from name (only if slug is empty or matches old name)
      const slug = value
        .toLowerCase()
        .replace(/[^a-z0-9\s-]/g, '')
        .replace(/\s+/g, '-')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: 'Success',
        description: 'Embossing option updated successfully'
      });
      ...
      toast({
        title: 'Error',
        description: error.message || 'Failed to update embossing option',
        variant: 'destructive'
      });
    } finally {
      setSaving(false);
      ...
      <AdminLayout>
        <div className="min-h-screen bg-gradient-to-br from-mint/20 via-white to-sage/10 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="mt-4 text-botanical/70">Loading embossing option...</p>
          </div>
        </div>
      </AdminLayout>
    );
      ...
      <div className="min-h-screen bg-gradient-to-br from-mint/20 via-white to-sage/10">
        <div className="max-w-4xl mx-auto p-6 space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl md:text-3xl font-heading text-forest">Edit Embossing Option</h1>
              <p className="text-sm text-botanical/80 font-medium mt-1">
                Update embossing font/style option
              </p>
            </div>
              ...
              <CardContent className="space-y-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    required
                  />
                  <p className="text-xs text-botanical/70 mt-1">URL-friendly identifier</p>
                </div>
