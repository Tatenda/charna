...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // For Next.js API routes, we don't have persistent memory, so we return empty
    ...
    return res.status(200).json({ 
      checkouts: [],
      message: "No recent checkouts found. Checkout IDs are not persisted in Next.js API routes."
    });
  } catch (error) {
