...
const HeroDatabase = () => {
  const { section, loading, error } = useLandingPageSection('hero');

  if (loading) {
    return (
      <section className="relative min-h-[60vh] sm:min-h-[70vh] md:h-[85vh] flex items-center overflow-hidden pt-16 md:pt-0 bg-gray-100 animate-pulse">
  ...
  if (error || !section) {
    ...
    // Fallback to hardcoded hero if database fails
    return (
  ...
  const ctaText = settings.ctaText || 'Shop Bags';
  const ctaLink = settings.ctaLink || '/browse?category=work';
  ...
  return (
    <section className="relative min-h-[60vh] sm:min-h-[70vh] md:h-[85vh] flex items-center overflow-hidden pt-16 md:pt-0">
      ...
      {heroImage && (
        <div className="absolute inset-0" style={{
          backgroundImage: `url(${getImagePath(heroImage.imageUrl)})`,
          backgroundSize: 'cover',
          backgroundPosition: 'center',
          backgroundRepeat: 'no-repeat'
        }}></div>
      ...
      <div className="container mx-auto px-4 text-center relative z-10">
        <div className="max-w-xs sm:max-w-xl md:max-w-4xl mx-auto space-y-6 md:space-y-8 mt-16 sm:mt-20 md:mt-0">
          <p className="text-lg sm:text-xl lg:text-2xl text-stone-200 leading-relaxed max-w-xl md:max-w-2xl mx-auto">
            {section.title || 'Handcrafted, beautiful and affordable bags'}
          </p>
          <div className="flex justify-center">
            <Link
              href={ctaLink}
